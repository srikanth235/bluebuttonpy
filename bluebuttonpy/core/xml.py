...
def parse(data):
    ...
    try:
        ...
        logging.info(f"BB Error: XML syntax error: {e}")
        return None
...
class _Element:
    ...
    def template(self, template_id):
        ...
        */
        ...
        if el is None:
            ...
            for parent in self._element.iter():
                for child in parent:
                    if child == el:
    ...
    def val(self):
        ...
        if not strip_whitespace(text_context):
            content_id = None
    ...
    def _wrap_element(self, element):
        if issubclass(type(element), list):
            return wrappers.ListWrapper([_Element(e, self._root) for e in element])
        else:
...
def _text_content(element):
    ...
    if element.tail is not None:
        portions.append(strip_whitespace(element.tail))
